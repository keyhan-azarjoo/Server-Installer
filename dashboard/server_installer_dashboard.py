...
"""
    ...
    entrypoint_lines = [
        ...
        "",
        "# IMPORTANT: Do NOT add agent/defaultModel/models keys to openclaw.json",
        "# OpenClaw validates its config schema strictly — unknown keys crash the gateway.",
        "# The model is set per-session in the UI (Settings > AI & Agents).",
        "",
        "# Fix any corrupted config from previous installs",
        "OC_CFG=/root/.openclaw/openclaw.json",
        "if [ -f $OC_CFG ]; then",
        "  echo 'Cleaning invalid keys from openclaw.json...'",
        "  python3 << 'PYEOF'",
        "import json",
        ...
        "except: cfg = {}",
        "changed = False",
        "for bad_key in ['agent', 'defaultModel', 'defaultProvider']:",
        "  if bad_key in cfg:",
        "    del cfg[bad_key]",
        "    changed = True",
        "if 'models' in cfg and 'ollama' in cfg.get('models', {}):",
        "  del cfg['models']['ollama']",
        "  if not cfg['models']: del cfg['models']",
        "  changed = True",
        "if changed:",
        "  json.dump(cfg, open(cfg_path, 'w'), indent=2)",
        "  print('Removed invalid keys from openclaw.json')",
        "else:",
        "  print('openclaw.json is clean')",
        "PYEOF",
        "fi",
        "",
        "# Delete existing sessions so new session uses fresh defaults",
        ...
        f'echo ""',
        f'echo "IMPORTANT: To use Ollama instead of Claude/OpenAI:"',
        f'echo "  1. Open the dashboard URL above"',
        f'echo "  2. Go to Settings (gear icon) > AI & Agents"',
        f'echo "  3. Change the model to: ollama/{llm_model}"',
        f'echo "  Ollama auth is pre-configured in auth-profiles.json"',
        f'echo "============================================="',
        "",
