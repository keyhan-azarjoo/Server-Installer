...
"""
    ...
    entrypoint_lines = [
        ...
        "",
        "# OpenClaw auto-discovers Ollama at 127.0.0.1:11434 ONLY.",
        "# If Ollama is elsewhere (user URL or host.docker.internal), proxy it to localhost.",
        "if ! curl -sf http://127.0.0.1:11434/api/tags >/dev/null 2>&1; then",
        "  # Extract host:port from OLLAMA_HOST URL (strip http:// or https://)",
        "  OLLAMA_TARGET=$(echo \"$OLLAMA_HOST\" | sed 's|^https\\?://||' | sed 's|/$||')",
        "  # If no port specified, default to 11434",
        '  echo "$OLLAMA_TARGET" | grep -q ":" || OLLAMA_TARGET="${OLLAMA_TARGET}:11434"',
        "  # Extract just the host (without port) for trying direct API",
        "  OLLAMA_HOST_ONLY=$(echo \"$OLLAMA_TARGET\" | cut -d: -f1)",
        "  # Try in order: user URL, same host on port 11434, host.docker.internal",
        "  PROXY_TARGET=''",
        "  if [ -n \"$OLLAMA_TARGET\" ] && curl -sf http://${OLLAMA_TARGET}/api/tags >/dev/null 2>&1; then",
        "    PROXY_TARGET=$OLLAMA_TARGET",
        "  elif [ -n \"$OLLAMA_HOST_ONLY\" ] && curl -sf http://${OLLAMA_HOST_ONLY}:11434/api/tags >/dev/null 2>&1; then",
        "    PROXY_TARGET=${OLLAMA_HOST_ONLY}:11434",
        "  elif curl -sf http://host.docker.internal:11434/api/tags >/dev/null 2>&1; then",
        "    PROXY_TARGET='host.docker.internal:11434'",
        "  fi",
        "  if [ -n \"$PROXY_TARGET\" ]; then",
        "    echo \"Ollama at $PROXY_TARGET — starting socat proxy to 127.0.0.1:11434\"",
        "    socat TCP-LISTEN:11434,fork,reuseaddr TCP:${PROXY_TARGET} &",
        "    sleep 1",
        "    echo 'socat proxy started'",
        "  else",
        "    echo 'WARNING: Ollama not reachable at $OLLAMA_TARGET or host.docker.internal:11434'",
        "  fi",
        "fi",
        "",
        "# Generate SSL cert",
        ...
        "# Disable device pairing for remote Control UI access",
        ...
        "fi",
        "",
        "# Write auth-profiles.json in CORRECT versioned format",
        ...
        '}',
        'AUTHEOF',
        "echo 'Auth profiles written (versioned format):'",
        "cat $AGENT_DIR/auth-profiles.json",
        "",
        ...
        "fi",
        "",
        "# Clear sessions and restart gateway with OLLAMA_API_KEY",
        "echo 'Clearing old sessions...'",
        "rm -rf /root/.openclaw/agents/main/sessions 2>/dev/null || true",
        "rm -rf /root/.openclaw/sessions 2>/dev/null || true",
        "",
        "echo 'Restarting gateway with Ollama enabled...'",
        "kill $GW_PID 2>/dev/null || true",
        "sleep 2",
        f"OLLAMA_API_KEY=ollama-local OLLAMA_HOST=$OLLAMA_API_URL openclaw gateway --allow-unconfigured --bind loopback --port {gw_internal_port} --verbose &",
        "GW_PID=$!",
        "sleep 5",
        "",
        "# Set default model to Ollama using the correct CLI command",
        f"echo 'Setting default model to ollama/{llm_model}...'",
        f"openclaw models set 'ollama/{llm_model}' 2>&1 || echo 'models set command not available'",
        "",
        ...
        f'echo "Default model: ollama/{llm_model}"',
        f'echo "============================================="',
        "",
        ...
        "nginx -t -c /app/nginx.conf 2>&1 || echo 'nginx config test FAILED'",
        "nginx -c /app/nginx.conf 2>&1 &",
        "sleep 2",
        f"echo 'Testing port {http_port}...'",
        f"curl -sk https://127.0.0.1:{http_port}/ 2>&1 | head -5 || echo 'curl test failed'",
        "",
        "wait $GW_PID",
    ]
