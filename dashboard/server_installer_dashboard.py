...
"""
    ...
    entrypoint_lines = [
        ...
        "fi",
        "",
        "# Clean old agent config and sessions that may default to anthropic",
        ...
        "",
        "# Set Ollama env vars so OpenClaw discovers it",
        ...
        "",
        "# Resolve Ollama URL — use env var set earlier (supports remote or local)",
        "OLLAMA_RESOLVED=${OLLAMA_HOST:-http://127.0.0.1:11434}",
        f"echo \"Ollama URL: $OLLAMA_RESOLVED\"",
        "",
        "# Write auth-profiles.json — tell OpenClaw about Ollama provider",
        ...
        "",
        "# Set default model in openclaw.json (the REAL config file)",
        "# OpenClaw uses openclaw.json, NOT config.yaml",
        f"echo 'Setting default model to ollama/{llm_model} in openclaw.json...'",
        "openclaw config set agent.model \"ollama/" + llm_model + "\" 2>&1 || true",
        "openclaw config set agent.provider ollama 2>&1 || true",
        "openclaw config set agent.defaultModel \"ollama/" + llm_model + "\" 2>&1 || true",
        "openclaw config set defaultModel \"ollama/" + llm_model + "\" 2>&1 || true",
        "openclaw config set defaultProvider ollama 2>&1 || true",
        "openclaw config set models.ollama.baseUrl \"$OLLAMA_RESOLVED\" 2>&1 || true",
        "",
        "# Patch openclaw.json directly to set model (belt and suspenders)",
        "OC_CFG=/root/.openclaw/openclaw.json",
        "if [ -f $OC_CFG ]; then",
        "  echo 'Patching openclaw.json directly...'",
        f"  python3 - \"$OLLAMA_RESOLVED\" << 'PYEOF'",
        "import json, sys",
        "ollama_url = sys.argv[1] if len(sys.argv) > 1 else 'http://127.0.0.1:11434'",
        f"model = 'ollama/{llm_model}'",
        ...
        "cfg.setdefault('models', {}).setdefault('ollama', {})",
        "cfg['models']['ollama']['baseUrl'] = ollama_url",
        "cfg['models']['ollama']['provider'] = 'ollama'",
        "json.dump(cfg, open(cfg_path, 'w'), indent=2)",
        "print('openclaw.json patched: model=' + model + ' url=' + ollama_url)",
        "PYEOF",
        "  cat $OC_CFG",
        "fi",
        "",
        "# Delete existing sessions so new ones use the correct model",
        "echo 'Clearing old sessions (will use new model on reconnect)...'",
        "rm -rf /root/.openclaw/agents/main/sessions 2>/dev/null || true",
        "rm -rf /root/.openclaw/sessions 2>/dev/null || true",
        "",
        ...
        "sleep 2",
        f"OLLAMA_HOST=${{OLLAMA_HOST}} openclaw gateway --allow-unconfigured --bind loopback --port {gw_internal_port} --verbose &",
        "GW_PID=$!",
        "sleep 5",
        "",
