...
"""
    ...
    entrypoint_lines = [
        ...
        "",
        "# Register Ollama provider using 'openclaw configure'",
        "# The OLLAMA_API_KEY env var alone doesn't work — must run configure",
        "echo 'Running openclaw configure to register Ollama provider...'",
        "echo '' | OLLAMA_API_KEY=ollama-local openclaw configure 2>&1 || true",
        "echo 'y' | OLLAMA_API_KEY=ollama-local openclaw configure 2>&1 || true",
        "# Also try the provider-specific configure",
        "OLLAMA_API_KEY=ollama-local openclaw configure --provider ollama 2>&1 || true",
        "OLLAMA_API_KEY=ollama-local openclaw provider add ollama 2>&1 || true",
        "OLLAMA_API_KEY=ollama-local openclaw providers add ollama --key ollama-local 2>&1 || true",
        "# Check env from running process perspective",
        "echo 'Checking env vars visible to openclaw:'",
        "OLLAMA_API_KEY=ollama-local openclaw doctor 2>&1 | head -20 || true",
        "",
        "# Generate SSL cert",
        ...
        f"const child = spawn(bin, ['gateway', '--allow-unconfigured', '--bind', 'loopback', '--port', '{gw_internal_port}', '--verbose'], {{",
        "  stdio: 'inherit',",
