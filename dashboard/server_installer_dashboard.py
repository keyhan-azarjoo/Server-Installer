...
"""
    ...
    entrypoint_lines = [
        ...
        f"OLLAMA_MODEL_ID='ollama/{llm_model}'",
        f"OLLAMA_BASE_URL='{ollama_url or 'http://127.0.0.1:11434'}'",
        "openclaw config set agent.model $OLLAMA_MODEL_ID 2>/dev/null || true",
        "openclaw config set agent.modelProvider ollama 2>/dev/null || true",
        "openclaw config set models.ollama.provider ollama 2>/dev/null || true",
        "openclaw config set models.ollama.baseUrl $OLLAMA_BASE_URL 2>/dev/null || true",
        f"openclaw config set models.ollama.model {llm_model} 2>/dev/null || true",
        "",
        "# Generate SSL cert",
        ...
        "",
        "# Configure agent model to use Ollama",
        f"echo 'Configuring agent to use Ollama model: {llm_model}'",
        "",
        "# Method 1: Use openclaw models CLI to add Ollama provider",
        f"openclaw models add ollama --provider ollama --base-url {ollama_url or 'http://127.0.0.1:11434'} 2>/dev/null || true",
        f"openclaw models set-default ollama/{llm_model} 2>/dev/null || true",
        "",
        "# Method 2: Set agent config via CLI",
        f"openclaw agents config main set model ollama/{llm_model} 2>/dev/null || true",
        f"openclaw config set agent.model ollama/{llm_model} 2>/dev/null || true",
        f"openclaw config set agent.defaultModel ollama/{llm_model} 2>/dev/null || true",
        "",
        "# Method 3: Write auth profile for Ollama (no API key needed)",
        ...
        f"d['ollama'] = {{'provider': 'ollama', 'baseUrl': '{ollama_url or 'http://127.0.0.1:11434'}', 'apiKey': 'ollama'}}",
        "with open(p, 'w') as f: json.dump(d, f, indent=2)",
        "print('Auth profile written for Ollama')",
        '" 2>/dev/null || true',
        "",
        "# Method 4: Set OLLAMA_HOST env var",
