(() => {
  ...
  ns.pages["agent-openclaw"] = function renderOpenClawPage(p) {
    ...
    var commonFields = [
      ...
      { name: "OPENCLAW_LLM_PROVIDER", label: "LLM Provider", type: "select", options: ["ollama (local)", "lmstudio (local)", "openai", "anthropic"], defaultValue: "ollama (local)", placeholder: "Select LLM" },
      ...
      { name: "OPENCLAW_OLLAMA_KEY", label: "Ollama API Key", defaultValue: "ollama-local", placeholder: "ollama-local (any value enables Ollama)" },
      ...
      { name: "OPENCLAW_LMSTUDIO_KEY", label: "LM Studio API Key", defaultValue: "lmstudio-local", placeholder: "lmstudio-local (any value enables LM Studio)" },
