...
function App() {
  ...
  const [openclawPageServices, setOpenclawPageServices] = React.useState([]);
  ...
  const [openclawInfoState, setOpenclawInfoState] = React.useState(null);
  ...
  const loadOpenclawServices = React.useRef(async () => {});
  ...
  const loadOpenclawInfo = React.useRef(async () => {});
  ...
  loadOpenclawServices.current = async () => loadServiceScope("openclaw", setOpenclawPageServices);
  ...
  loadOpenclawInfo.current = async () => loadScopedStatus("openclaw", setOpenclawInfoState);
  ...
  const buildDotnetDockerManualHelper = React.useCallback(() => ({
    ...
    intro: "Step-by-step manual setup for running a published .NET API with Docker and Nginx, matching the standalone manual.",
    sections: [
      {
        title: "Folder Structure",
        body: "Use this layout for a published Linux build.",
        code: [
      ...
      {
        title: "Step 1: Go to dockerfiles",
        body: "Open the folder where the Docker-related files will live.",
        code: [
      ...
      },
      {
        ...
        body: "Create `Dockerfile` and save it without a `.txt` extension.",
        code: [
          "notepad Dockerfile",
          "",
      ...
      {
        title: "Step 3: Create nginx.conf",
        body: "Create the Nginx reverse proxy config.",
        code: [
          "notepad nginx.conf",
          "",
          "events {}",
      ...
      {
        title: "Step 4: Create docker-compose.yml",
        body: "Create the compose file beside `Dockerfile` and `nginx.conf`.",
        code: [
      ...
      {
        title: "Step 5: Create certs",
        body: "Generate the self-signed certificate if you do not already have it.",
        code: [
          "mkdir certs",
      ...
      {
        title: "Step 6: Build and run",
        body: "Start the API container and Nginx together.",
        code: [
      ...
      {
        title: "Step 7: Verify",
        body: "Both containers should be running.",
        code: [
          "docker ps",
        ].join("\n"),
        language: "powershell",
      ...
      {
        title: "Step 8: Test",
        body: "Use the real API route, not `/`, if your app does not expose a root endpoint.",
        code: [
          "HTTP:",
      ...
      {
        title: "Notes",
        body: "This is the same working pattern used in the repo manual.",
        code: [
          "404 on / is normal if the API has no root endpoint.",
          "HTTPS browser warning is normal with a self-signed certificate.",
          "Flow: browser -> Nginx -> http://api:8080 -> .NET API",
          "Reuse the same pattern for other published .NET APIs by changing the DLL name and ports.",
        ].join("\n"),
        language: "text",
      },
    ],
  ...
  const buildS3ManualHelper = React.useCallback(() => {
    return {
      ...
      intro: "Step-by-step manual MinIO + Nginx Docker setup, matching the standalone S3 manual.",
      sections: [
        {
          title: "Folder",
          body: "Use this folder for the full setup.",
          code: [
            "C:\\keyhan\\API\\S3",
          ].join("\n"),
          language: "text",
        },
        {
          ...
          body: "Start from a clean directory so old containers and files do not conflict.",
          code: [
            "cd C:\\keyhan\\API\\S3",
            "docker compose down",
        ...
        {
          title: "Step 1: Create structure",
          body: "Create the required folders.",
          code: [
            "mkdir data",
            "mkdir certs",
            "mkdir nginx",
        ...
        {
          title: "Step 2: Generate SSL",
          body: "Generate certificates without installing OpenSSL locally.",
          code: [
        ...
        {
          title: "Step 3: Create nginx\\nginx.conf",
          body: "Proxy both HTTP and HTTPS to the MinIO console on port `9001`.",
          code: [
            "notepad nginx\\nginx.conf",
            "",
            "events {}",
        ...
        {
          title: "Step 4: Create docker-compose.yml",
          body: "Run MinIO and Nginx together with the working ports and credentials.",
          code: [
            "notepad docker-compose.yml",
            "",
            "services:",
        ...
        {
          title: "Step 6: Access",
          body: "Open the MinIO UI through Nginx.",
          code: [
            "HTTP:",
            "http://localhost",
            "",
        ...
        {
          title: "Login",
          body: "Use these credentials in the UI.",
          code: [
            "Username: admin",
            "Password: admin123",
          ].join("\n"),
        ...
        },
        {
          ...
          body: "The full MinIO UI should be available.",
          code: [
            "Buckets",
            "Identity",
            "Users",
        ...
        },
        {
          ...
          body: "This mirrors the working manual setup exactly.",
          code: [
            "MinIO API -> 9000",
            "MinIO UI -> 9001",
            "Nginx handles HTTP and HTTPS in front of MinIO",
            "The root path is proxied to the MinIO UI",
          ].join("\n"),
          language: "text",
        },
        {
          title: "Optional: expose the S3 API through Nginx",
          body: "Add this later if you also want the API behind the same reverse proxy.",
          code: [
            "location /s3/ {",
  ...
  const buildMongoDockerManualHelper = React.useCallback(() => ({
    ...
    intro: "Step-by-step manual MongoDB + Mongo Express + Nginx Docker setup, matching the standalone manual.",
    sections: [
      {
        title: "Ports",
        body: "This setup uses these fixed ports.",
        code: [
          "HTTP: 8721",
          "HTTPS: 8722",
          "MongoDB: 27017",
      ...
      {
        title: "Step 0: Clean start",
        body: "Create the folder and remove old files before redeploying.",
        code: [
      ...
      {
        title: "Step 2: Generate SSL",
        body: "Generate the certificate files.",
        code: [
          "docker run --rm -v ${PWD}/certs:/certs alpine sh -c \"apk add --no-cache openssl && openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /certs/private.key -out /certs/public.crt -subj '/CN=localhost'\"",
        ].join("\n"),
        language: "powershell",
      ...
      {
        title: "Step 3: Create nginx\\nginx.conf",
        body: "Proxy Mongo Express through HTTP and HTTPS on the chosen ports.",
        code: [
          "notepad nginx\\nginx.conf",
          "",
          "events {}",
      ...
      {
        title: "Step 4: Create docker-compose.yml",
        body: "Run MongoDB, Mongo Express, and Nginx together.",
        code: [
          "notepad docker-compose.yml",
          "",
          "services:",
      ...
      {
        title: "Step 5: Run",
        body: "Start all three containers.",
        code: [
          "docker compose up -d",
        ].join("\n"),
        language: "powershell",
      },
      {
        ...
        body: "Use these addresses and credentials.",
        code: [
          "UI",
          "http://localhost:8721",
      ...
      },
      {
        ...
        body: "After setup, the folder should look like this.",
        code: [
          "C:\\keyhan\\API\\Mongo",
          "│",
          "├── data\\",
      ...
      {
        title: "Result",
        body: "This is the final expected outcome.",
        code: [
          "MongoDB running",
          "UI via HTTP and HTTPS",
  ...
  const openclawSoftware = openclawInfoState?.software || {};
  ...
  const openclawService = openclawSoftware.openclaw_service || software.openclaw_service || {};
  ...
  const commonProps = {
    ...
    ollamaPageServices, lmstudioPageServices, openclawPageServices, tgwuiPageServices, comfyuiPageServices, whisperPageServices, piperPageServices,
    ...
    ollamaService, lmstudioService, openclawService, tgwuiService, comfyuiService, whisperService, piperService,
    ...
    loadOllamaServices, loadLmstudioServices, loadOpenclawServices, loadTgwuiServices, loadComfyuiServices, loadWhisperServices, loadPiperServices,
    loadMongoInfo, loadS3Info, loadDotnetInfo, loadDockerInfo, loadProxyInfo,
    loadPythonInfo, loadWebsiteInfo, loadSam3Info,
    loadOllamaInfo, loadLmstudioInfo, loadOpenclawInfo, loadTgwuiInfo, loadComfyuiInfo, loadWhisperInfo, loadPiperInfo,
