...
"""
    ...
    entrypoint_lines = [
        ...
        '# Detect the Docker host IP so we can set exact allowedOrigins (["*"] is NOT a wildcard in OpenClaw)',
        'DOCKER_HOST_IP=$(getent hosts host.docker.internal 2>/dev/null | awk \'{print $1}\' | head -1 || hostname -I 2>/dev/null | awk \'{print $1}\' || echo "127.0.0.1")',
        f'echo "Setting allowedOrigins for host IP: $DOCKER_HOST_IP port {http_port}"',
        f'openclaw config set gateway.controlUi.allowedOrigins "[\\\"https://$DOCKER_HOST_IP:{http_port}\\\",\\\"https://127.0.0.1:{http_port}\\\",\\\"https://localhost:{http_port}\\\",\\\"http://$DOCKER_HOST_IP:{http_port}\\\",\\\"http://127.0.0.1:{http_port}\\\",\\\"http://localhost:{http_port}\\\"]" 2>/dev/null || true',
        ...
        "  if not chosen:",
        "    chosen = models[0]",
        "ollama_models = ['ollama/' + m for m in models]",
        ...
        "",
        "# Populate model picker with ALL Ollama models (when reachable)",
        "if [ -f /tmp/ollama-models.json ]; then",
        "  openclaw config set agents.defaults.models \"$(cat /tmp/ollama-models.json)\" 2>/dev/null || true",
        "fi",
        "",
